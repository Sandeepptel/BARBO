...
(
    ...
    constraint fk_state_city foreign key (state_id) references states (state_id)
);
...
(
    ...
    starting_year varchar(6) not null,
    end_time varchar(10) ,
    city_id int,
    foreign key fk_barber_salon references salon (salon_id),
    foreign key fk_barber_city references cites (city_id)
);

alter table add constraint fk_barber_city  foreign key (city_id) references cites (city_id);
alter table add constraint salon foreign key (salon_id) references salon (salon_id);

create table salon(
    salon_id int not null auto_increment primary key ,
    salon_type varchar(8) not null
);

insert into salon (salon_type) values
('men'),('women'),('both');
...
('hair masks'),('facial'),('makup'),('jacuzzi'),('maincure'),('pedicure'),('hair iron'),('bridal package'),
('hair rebonding'),('hair treatment'),('skin treatment'),('hair straightening'),('membership offered'),('home services'),
('threading services'),('hair treatment'),('parking available'),('wifi'),('ac');

-- services ...
create table barbers_services 
(
    barber_service_id int not null auto_increment primary key,
    barber_id int not null,
    constraint fk_barber_barberService foreign key (barber_id) references barbers (barber_id),
    service_id int not null,
    constraint fk_service_barberService foreign key (service_id) references services (service_id)
);

alter table add constraint fk_barberId_Services foreign key (barber_id) references barbers (barber_id);
alter table add constraint fk_services_Id foreign key (service_id) references services (service_id);
...
(
    ...
    barber_service_id int not null
);
alter table charges add constraint fk_charges_barberService foreign key (barber_service_id) references barbers_services (barber_service_id);


-- images ... 
create table barber_images
(
    barber_image_id int auto_increment not null primary key,
    salon_image_path varchar(150) not null,
    barber_id int not null,
    constraint fk_barberimages_barber foreign key (barber_id) references barbers (barber_id)
);
...
INSERT INTO `cites` (`cityId`, `name`, `state_id`) VALUES
